...
if ($action=="modify")
{
	...
	elseif($_POST['step']==2)
	{
		...
		if($gp_ifportait && !empty($proownportait[0]))
		{
			...
			if (!preg_match("/^[0-9]{2,3}$/",$proownportait[1]) || !preg_match("/^[0-9]{2,3}$/",$proownportait[2]) || $proownportait[1]>215 || $proownportait[1]<0 || $proownportait[2]>250 ||$proownportait[2]<0) {$msg_info="你自定义的图片必须在(0-215)*(0-250)的大小范围里"; $check=0;}
