...
{
	alert("你的信息已经有 "+theform.atc_content.value.length+" 字节.");
}
...
function rming() {
	if (helpmode) {
		alert("RM音乐标记\n插入一个RM链接标记\n使用方法: [rm]http:\/\/www.123.com\/rm\/php.rm[/rm]");
	} else if (nomode || document.selection && document.selection.type == "Text") {
		AddTxt="[rm]"+text+"[/rm]";
		AddText(AddTxt);
	} else {
...
function image() {
	if (helpmode){
		alert("图片标记\n插入图片\n用法: [img]http://www.hao123.com/logo.gif[/img]");
	} else if (nomode || document.selection && document.selection.type == "Text") {
		AddTxt="[img]"+text+"[/img]";
		AddText(AddTxt);
	} else {
...
function wmv() {
	if (helpmode){
		alert("wmv标记\n插入wmv\n用法: [wmv]http:\/\/www.123.com\/wmv\/php.wmv[/wmv]");
	} else if (nomode || document.selection && document.selection.type == "Text") {
		AddTxt="[wmv]"+text+"[/wmv]";
		AddText(AddTxt);
	} else {
