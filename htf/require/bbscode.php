...
function convert($message, $allow, $type = "post")
{
    ...
    $message = str_replace("[:htfupload]", $attachpath, $message);
    $message = str_replace("[:htffile]", $picpath, $message);
    ...
    // Image handling - PHP 8 fix: replace /e modifier with preg_replace_callback
    if ($allow['pic']) {
        $message = preg_replace_callback("/\[img\](.+?)\[\/img\]/is", function($matches) {
            return cvpic($matches[1]);
        }, $message);
    } else {
        $message = preg_replace("/(\[img\])(\S+?)(\[\/img\])/is",
            "<img src='$imgpath/$stylepath/file/img.gif' align='absbottom'> <a target=_blank href='\\2'>images: \\2</a>",
            $message);
    }

    // Flash handling
    if ($allow['flash']) {
        $message = preg_replace("/(\[flash=)(\S+?)(\,)(\S+?)(\])(\S+?)(\[\/flash\])/is",
            "<OBJECT CLASSID=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" WIDTH=\\2 HEIGHT=\\4><PARAM NAME=MOVIE VALUE=\\6><PARAM NAME=PLAY VALUE=TRUE><PARAM NAME=LOOP VALUE=TRUE><PARAM NAME=QUALITY VALUE=HIGH><EMBED SRC=\\6 WIDTH=\\2 HEIGHT=\\4 PLAY=TRUE LOOP=TRUE QUALITY=HIGH></EMBED></OBJECT><br />[<a target=_blank href=\\6>全屏观看</a>] ",
            $message);
    } else {
        $message = preg_replace("/(\[flash=)(\S+?)(\,)(\S+?)(\])(\S+?)(\[\/flash\])/is",
            "<img src='$imgpath/$stylepath/file/music.gif' align='absbottom'> <a target=_blank href=\\6>flash: \\6</a>",
            $message);
    }
    ...
    if($type == "post")
    {
        ...
        $act = "<font color=red>特殊动作</font>";
        $count = count($motion);
        for($i = 1; $i <= $count; $i++)
            $message = str_replace("/{$motion[$i][0]}", "$act $tpc_author {$motion[$i][1]}<br><img src=$imgpath/post/act/{$motion[$i][2]} ><br>", $message);

        // PHP 8 fix: replace /e modifier with preg_replace_callback
        if ($fid_sell != 2 && strpos($message, "[sell") !== false && strpos($message, "[/sell]") !== false)
        {
            $message = preg_replace_callback("/\[sell=(.+?)\](.+?)\[\/sell\]/is", function($matches) {
                return sell($matches[1], $matches[2]);
            }, $message);
        }
        if ($fid_post != 2 && strpos($message, "[post]") !== false && strpos($message, "[/post]") !== false)
        {
            $message = preg_replace_callback("/\[post\](.+?)\[\/post\]/is", function($matches) {
                return post($matches[1]);
            }, $message);
        }
        if ($fid_hide != 2 && strpos($message, "[hide") !== false && strpos($message, "[/hide]") !== false)
        {
            $message = preg_replace_callback("/\[hide=(.+?)\](.+?)\[\/hide\]/is", function($matches) {
                return hiden($matches[1], $matches[2]);
            }, $message);
        }
        if ($allow['mpeg']) {
            ...
            $message = preg_replace("/(\[wmv\])(\S+?)(\[\/wmv\])/is",
                "<img src='$imgpath/$stylepath/file/music.gif' align='absbottom'> <a target=_blank href='\\2'>\\2</a>",
                $message);
            $message = preg_replace("/(\[rm\])(\S+?)(\[\/rm\])/is",
                "<img src='$imgpath/$stylepath/file/music.gif' align='absbottom'> <a target=_blank href='\\2'>\\2</a>",
                $message);
        }
        if ($allow['iframe']) {
            $message = preg_replace("/\[iframe\]\s*(\S+?)\s*\[\/iframe\]/is",
                "<IFRAME SRC=\\1 FRAMEBORDER=0 ALLOWTRANSPARENCY=true SCROLLING=YES WIDTH=97% HEIGHT=340></IFRAME>",
                $message);
        } else {
            $message = preg_replace("/(\[iframe\])(\S+?)(\[\/iframe\])/is",
                "Iframe 关闭: <a target=_blank href='\\2'>\\2</a>",
                $message);
        }
        if (strpos($message, "[code]") !== false && strpos($message, "[/code]") !== false)
        {
            $message = preg_replace_callback("/\[code\](.+?)\[\/code\]/is", function($matches) {
                return phpcode($matches[1]);
            }, $message);
        }
    }
    ...
    // Word filter - PHP 8 fix: replace each() with foreach
...
function cvpic($url, $style = '')
{
    ...
    if(strpos($url, $db_bbsurl) !== false)
    {
        ...
        if($urlarray[0] != '') {
            strpos($urlarray[1], '.') !== false ? $urlarray[0] = $attachpath : $urlarray[0] = $picpath;
        } else {
            strpos($urlarray[2], '.') !== false ? $urlarray[1] = $attachpath : $urlarray[1] = $picpath;
        }
        $urldb[1] = implode("/", $urlarray);
        $url = implode("$db_bbsurl", $urldb);
    }
...
function sell($moneycost, $code)
{
    ...
    if($moneycost < 0)
        $moneycost = 0;
    // PHP 8 fix: replace ereg() with preg_match()
    if ($moneycost && !preg_match("/^[0-9]{0,}$/", $moneycost))
        $moneycost = 0;
...
function post($code)
{
    ...
    if($admin_check == 1 || ($authortemp && in_array($htfid, $authortemp)))
        ...
        $tpc_download = '';
        $printcode = '<br><br><table cellpadding=0 cellspacing=0 border=0 WIDTH=94% bgcolor=#000000 align=center><tr><td><table width=100% cellpadding=5 cellspacing=1 border=0><TR><TD BGCOLOR=#EFF3F9>本内容被作者设定为<font color=red>隐藏</font>,需要<font color=red>回复</font>才能看到</table></table><br><br>';
    }
    return $printcode;
}
...
function hiden($rvrc, $code)
{
    ...
    if($groupid != 'guest')
    {
        ...
        if($userrvrc < $rvrc && $admin_check != 1)
            $printcode = "<table cellpadding=0 cellspacing=0 border=0 WIDTH=100% bgcolor=#000000 align=center><tr><td><table width=100% cellpadding=5 cellspacing=1 border=0><TR><TD BGCOLOR=#EFF3F9>[本内容被作者设定为<font color=red>隐藏</font>,除非你是管理员,需要{$rvrc}威望,你现在只有{$userrvrc}]</table></table>";
        else
            $printcode = "&nbsp;[本内容被作者隐藏,你拥有<font color=red>查看</font>权限:]<br><table cellpadding=0 cellspacing=0 border=0 WIDTH=100% bgcolor=#000000 align=center><tr><td><table width=100% cellpadding=5 cellspacing=1 border=0><TR><TD BGCOLOR=#EFF3F9>$code</table></table>";
    }
    else
    {
        $printcode = "对不起!你没有登录,请先<a href=login.php><font color=red>登录</font></a>.";
    }
    return $printcode;
}
